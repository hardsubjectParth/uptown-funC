  ...
  useEffect(() => {
    ...
    getUploadScopes(token)
      .then((result) => {
        ...
        setScope(result.scopes[0] ?? '')
      })
      .catch((cause) => {
        setUploadError(
          cause instanceof Error ? cause.message : 'Unable to load upload scopes',
        )
      })
  }, [token])
      ...
      <section className="knowledge-header">
        <div>
          ...
          <p className="dashboard-subtitle">
            Search information available within your authorized access scope.
          </p>
        </div>
      </section>
        ...
        <div className="section-heading">
          <div>
            ...
            <p className="helper-text">
              Uploaded documents become searchable organizational knowledge.
            </p>
          </div>
        </div>
      ...
      <section className="knowledge-results">
        ...
        {files.length > 0 ? (
          <div className="knowledge-result-list">
            {files.map((file) => (
              <article className="knowledge-result" key={file.id}>
                <div className="document-icon">
                  {file.metadata.mime_type?.includes('pdf') ? 'PDF' : 'DOC'}
                </div>
                ...
                <div className="document-info">
                  ...
                  <div className="document-meta">
                    <span>
                      {formatFileSize(file.metadata.size_bytes)}
                    </span>

                    <span className="meta-separator">·</span>

                    <span>
                      {file.metadata.mime_type ?? 'Unknown type'}
                    </span>
                  </div>
                </div>

                <span className="visibility-badge">
                  {file.metadata.visibility_tier
                    ? file.metadata.visibility_tier.charAt(0).toUpperCase() +
                    file.metadata.visibility_tier.slice(1)
                    : 'Authorized'}
                </span>
              </article>
            ))}
        ...
        <form onSubmit={handleSearch}>
          ...
          <div className="knowledge-search-row">
            <input
              ...
              placeholder="Search your authorized knowledge..."
              required
            />
