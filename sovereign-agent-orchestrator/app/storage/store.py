...
class Store:
    ...
    def add_message(self, message_id, conversation_id, role, content, citations=None):
        now = datetime.now(timezone.utc).isoformat()
        with self.engine.begin() as db:
            db.execute(text('INSERT INTO messages(id,conversation_id,role,content,citations,created_at) VALUES(:id,:conversation,:role,:content,:citations,:created)'), {'id': message_id, 'conversation': conversation_id, 'role': role, 'content': content, 'citations': json.dumps(citations or []), 'created': now})
