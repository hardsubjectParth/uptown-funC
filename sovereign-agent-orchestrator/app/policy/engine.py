...
class Policy:
    risks={'search_documents':0,'read_file':0,'write_file':1,'generate_docx':1,'generate_xlsx':1,'generate_pptx':1,'ingest_document':0,'list_sources':0,'export_report':1,'spreadsheet_profile':0,'redact_pii':1,'extract_tables':0,'ocr_document':0,'search_db':1,'send_email':2,'create_calendar_event':2,'run_python':1,'describe_image':0}
    ...
    def check(self,tool,user):
        ...
        if tool in ('generate_docx','generate_xlsx','generate_pptx'): return PolicyResult(Decision.REQUIRE_APPROVAL,'DOCUMENT_ARTIFACT_REQUIRES_REVIEW') if 'approver_demo' in {user.get('requested_role'), user.get('role')} else PolicyResult(Decision.ALLOW,'LOW_RISK_CONFIGURED')
